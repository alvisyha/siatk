                            ...
                            {isLoading ? (
                                ...
                                filteredTransaksi.map((tr) => (
                                    <tr key={tr.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-bold text-blue-800 text-right">
                                            {formatCurrency((tr.jumlah || 0) * (tr.harga || 0))}
                                        </td>
                                         <td className="px-6 py-4">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in duration-300">
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                {formData.barang_id && (() => {
                                    const lastTransaction = transaksiList.find(t => t.barang_id === formData.barang_id);
                                    if (lastTransaction) {
                                        return (
                                            <p className="mt-2 text-sm text-gray-600 flex items-center gap-1.5">
                                                <Banknote className="w-3.5 h-3.5 text-blue-500" />
                                                Harga Terakhir: <strong className="text-gray-900">{formatCurrency(lastTransaction.harga || 0)}</strong>
                                            </p>
                                        );
                                    }
                                    return null;
